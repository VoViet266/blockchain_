...
export const integrityCheck = async (req, res, next) => {
  console.log("Integrity Check:", req.params.id);
  try {
    ...
    if (!productId) {
      return res
        .status(400)
        .json({ message: "Không tìm thấy ID sản phẩm để xác thực." });
    }
    ...
    if (!result.isValid) {
      ...
      return res.status(409).json({
        success: false,
        message:
          "CẢNH BÁO: Dữ liệu sản phẩm này đã bị thay đổi trái phép so với Blockchain. Truy cập bị từ chối!",
        details: result.data,
      });
    }
    ...
    res
      .status(500)
      .json({ message: "Không thể xác thực với Blockchain: " + error.message });
  }
};
